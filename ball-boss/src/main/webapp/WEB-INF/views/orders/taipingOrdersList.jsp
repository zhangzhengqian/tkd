			...
			<div class="row">
				...
				<div class="col-md-12">
					<form id="orderForm" class="form-horizontal"
						action="${ctx }/orders/taipingList" method="post">
			...
			<div class="modal-content">
				...
				<div class="modal-footer">
					<a href="javascript:;" class="btn" data-dismiss="modal"
						...
						class="btn btn-primary alert-to-ok" onClick="lemiOrderCancel()"
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<a href="javascript:;" class="btn btn-primary alert-to-ok" onClick="lemiOrderAudi()" data-dismiss="modal" aria-hidden="true">确定</a>
				</div>
			</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		function lemiOrderCancel() {
			...
			bootbox.confirm('是否确认退款' + orderId + '订单?', function(result) {
				Util.getData(ctx + '/statium/lemiOrderCancel', function (data) {
					if (data.result) {
						myAlert(data.reason);
		...
		//调用太平取消订单接口
		function lemiOrderAudiReason(orderId,status) {
			$("#audiId").val(orderId);
			$("#audiStatus").val(status);
			if(status == 4){
				$("#audiOrderModel").modal();
			}else{
				lemiOrderAudi(orderId,status);
			}

		};
		function lemiOrderAudi(orderId,status) {
			...
			bootbox.confirm(tips, function(result) {
				Util.getData(ctx + '/statium/lemiOrderAudi', function (data) {
					if (data.result) {
						myAlert(data.reason);
						window.location.reload()
					} else {
		...
		function exportData() {
			var $form = $('#orderForm');
			$form.attr("action","${ctx }/orders/lemiexport");
			$form[0].submit();
		}

...
<input type="hidden" value="${zone.id }" name="id">
		...
		<div class="row">
			...
			<div class="col-md-8">
		      ...
		      <c:forEach items="${users}" var="user">
		      	<option <c:if test="${user.id==zone.userId}">selected="select"</c:if> value="${user.id }">${user.qiuyouno}-${user.phone }-${user.nickName }</option>
		      </c:forEach>
		      </select> (球友号－手机号－昵称)
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-md-8">
		      <div class="has-feedback" id="photo_content">
					...
					<c:set var="photoSize" value="${fn:length(zone.photo)}"></c:set>
					<c:forEach items="${zone.photo }" var="photo_" varStatus="stat">
						<div style="float:left;margin-right:10px;">
							<input id="photo${stat.count}" value="${photo_}" name="photo" type="hidden" />
							<img alt="" src="${photo_}" height="100" width="130" id="photo_img${stat.count}"/>
							<div aria-hidden="true" class="img_close">&times;</div>
						</div>
					</c:forEach>
					<c:forEach begin="${photoSize+1}" end="9" var="ind">
						<div style="float:left;margin-right:10px;">
							<input id="photo${ind}" name="photo" type="hidden" />
							<img alt="" src="" id="photo_img${ind}"/>
						</div>
					</c:forEach>
	         </div>
			</div>
		...
		<div class="row">
			...
			<div class="col-md-6">
		      <textarea rows="5" name="content" class="form-control">${zone.content}</textarea>
			</div>
		</div>
		<hr>
		<div class="row">
			...
			<div class="col-md-6">
		      <input  type="text" class="form-control input-sm" id="statiumName" value="${zone.statiumName }" placeholder="请输入所在场馆">
			  <input  type="hidden" class="form-control input-sm" id="statiumId" value="${zone.statiumId }" name="statiumId">
			</div>
		</div>
		<hr>
		<div class="row">
			...
			<div class="col-md-6">
		      ...
		      <c:forEach items="${labels}" var="label">
		      	<option <c:if test="${label.id==zone.labelId}">selected="select"</c:if> value="${label.id }">${label.name}</option>
		      </c:forEach>
		      </select>
			</div>
		</div>
		...
		<div class="row">
			...
			<input type="radio" <c:if test="${1==zone.sendType}">checked="checked"</c:if> name="sendType" value="1"> 立即发布
			<input type="radio" <c:if test="${0==zone.sendType}">checked="checked"</c:if> name="sendType" value="0"> 延时发布
			<input type="text" id="sendTime" style="margin-left:10px;display:none;" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH',firstDayOfWeek:1,minDate:'%y-%M-%d {%H+1}'})" value="${zone.tempTime }" class="form-control" name="tempTime">
			</div>
		</div>
		<hr>

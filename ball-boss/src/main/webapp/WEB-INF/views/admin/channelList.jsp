...
<div class="panel panel-default">
  ...
  <div class="panel-body"><!-- 右侧主体内容 -->
		...
		<div class="row">
			<div class="col-md-12">
				<form class="form-horizontal" action="${ctx }/admin/user/channelList" method="post">
		...
		<div class="row">
			<div class="col-md-12 text-right">
				<shiro:hasPermission name="channel:create">
	        		<a class="btn btn-sm btn-primary" href="${ctx }/admin/user/createChannel" ><span class="glyphicon glyphicon-plus"></span> 创建新用户</a>
	      		</shiro:hasPermission>	
			</div>
		</div>		
		<br>
		...
		<table id="contentTable" class="table table-bordered table-condensed table-hover">
			...
			<c:forEach items="${data.content}" var="user" varStatus="stat">
				<tr>
					...
					<td>
					  <a href="${ctx}/admin/user/updateChannel/${user.userId}" id="editLink-${user.userId}"> 修改</a>
					   <span class="cutline"></span> 
					  <a href="#" data="${user.nickname }" id="removeLink-${user.userId}" onclick="deleteById('${user.userId}')"> 删除</a>
					   <span class="cutline"></span>
					  <a href="${ctx}/admin/user/resetPwdChannel/${user.userId}" data="${user.nickname }" class="resetPwd" id="resetLink-${user.userId}"> 重置密码</a>
					</td>
				</tr>
			</c:forEach>
...
<script type="text/javascript">
  ...
  function getSelected() {
	...
	function deleteById(id) {
	  var $form = $('#actionForm');
	  $form.attr('action', '${ctx}/admin/user/deleteChannel/' + id);
	...
	function deleteBySelected() {
	  var $form = $('#actionForm');
	  $form.attr('action', '${ctx}/admin/user/deleteChannel');

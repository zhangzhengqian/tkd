  ...
  <div class="panel-body"><!-- 右侧主体内容 -->
		...
		<div class="row"><!-- 查询条件 -->
			<div class="col-md-12">
		      ...
		      <form class="form-horizontal" id="ssoUserForm" action="${ctx}/ssouser/user" method="post">
		         ...
		         <div class="form-group form-group-sm query-more">
		         	<label class="control-label col-md-1 sr-only" for="custName"></label>
		         	<div class="col-md-5">
			        	<input type="text" class="form-control input-sm" id="search_LIKE_cardId" name="search_LIKE_cardId" value="${param.search_LIKE_cardId }" placeholder="按用身份证号查询">
			       	</div>
	  				<div class="col-md-5">
		         ...
		         <div class="form-group form-group-sm query-more">
          			...
          			<div class="col-md-5">
            			<input type="text" class="form-control Wdate " id="search_GTE_registTime" name="search_GTE_registTime" value="${param.search_GTE_registTime }" placeholder="注册日期-开始" onfocus="WdatePicker({firstDayOfWeek:1,maxDate:'#F{$dp.$D(\'search_LTE_registTime\')||\'%y-%M-%d\'}'})">
          			</div>
          			<div class="col-md-5">
            			<input type="text" class="form-control Wdate " id="search_LTE_registTime" name="search_LTE_registTime" value="${param.search_LTE_registTime }" placeholder="注册日期-结束" onfocus="WdatePicker({firstDayOfWeek:1,maxDate:'%y-%M-%d',minDate:'#F{$dp.$D(\'search_GTE_registTime\')}'})">
          			</div>         
        		</div>
        		...
        		<div class="form-group form-group-sm query-more">
          			<lable class="control-label col-md-1 sr-only"></lable>
          			<div class="col-md-5">
            			<input type="text" class="form-control Wdate " id="search_GTE_createTime" name="search_GTE_createTime" value="${param.search_GTE_createTime }" placeholder="激活日期-开始" onfocus="WdatePicker({firstDayOfWeek:1,maxDate:'#F{$dp.$D(\'search_LTE_createTime\')||\'%y-%M-%d\'}'})">
          			</div>
          			<div class="col-md-5">
            			<input type="text" class="form-control Wdate " id="search_LTE_createTime" name="search_LTE_createTime" value="${param.search_LTE_createTime }" placeholder="激活日期-结束" onfocus="WdatePicker({firstDayOfWeek:1,maxDate:'%y-%M-%d',minDate:'#F{$dp.$D(\'search_GTE_createTime\')}'})">
          			</div>         
        		</div>
        		...
        		<div class="form-group form-group-sm query-more">
		         	<label class="control-label col-md-1 sr-only" for="custName"></label>
		         	<div class="col-md-5">
						<select class="form-control" name="search_EQ_property" id="search_EQ_property" >
							<option id="option1" value="" >--请选择用户类型--</option>
							<option id="option2" value="普通" <c:if test="${param.search_EQ_property eq '普通'}">selected='selected'</c:if> >--普通--</option>
							<option id="option3" value="教练" <c:if test="${param.search_EQ_property eq '教练'}">selected='selected'</c:if> >--教练--</option>
							<option id="option4" value="陪练" <c:if test="${param.search_EQ_property eq '陪练'}">selected='selected'</c:if> >--陪练--</option>
						</select>
			  		</div>
			       	<div class="col-md-5">
						<select class="form-control" name="search_EQ_sex" id="search_EQ_sex" >
							<option id="option5" value="" >--请选择性别--</option>
							<option id="option6" value="男" <c:if test="${param.search_EQ_sex eq '男'}">selected='selected'</c:if> >--男--</option>
							<option id="option7" value="女" <c:if test="${param.search_EQ_sex eq '女'}">selected='selected'</c:if> >--女--</option>
						</select>
			  		</div>
		         </div>
		         ...
		         <div class="form-group form-group-sm query-more">
		         	<label class="control-label col-md-1 sr-only" for="custName"></label>
		         	<div class="col-md-5">
						<select class="form-control" name="search_EQ_level" id="search_EQ_level" >
			       	...
			       	<div class="col-md-5">
						<select class="form-control" name="search_EQ_state" id="search_EQ_state" >
							...
							<option id="option14" value="1" <c:if test="${param.search_EQ_state ==1}">selected='selected'</c:if> >--正常--</option>
							<option id="option15" value="3" <c:if test="${param.search_EQ_state ==3}">selected='selected'</c:if> >--冻结--</option>
						</select>
			  		</div>
		         </div>
		         ...
		         <div class="form-group form-group-sm query-more">
		         	<label class="control-label col-md-1 sr-only" for="custName"></label>
		         	<div class="col-md-5">
						<select class="form-control" name="search_EQ_registSource" id="search_EQ_registSource" >
			       	...
			       	<div class="col-md-5">
						<select class="form-control" name="search_EQ_activeState" id="search_EQ_activeState" >
		         ...
		         <div class="form-group form-group-sm query-more">
			         ...
			         <div class="col-md-5 form-inline">
			          	<tags:zone name="search_LIKE_city" value="${param.search_LIKE_city }"  />
			         </div>
		         </div>
		        ...
		        <div class="form-group form-group-sm">
	  				<div class="col-md-12 text-center">
		       		...
		       		<input type="hidden" id="v_search_EQ_activeState" value="${param.search_EQ_activeState}">
			       	<input type="hidden" id="v_search_EQ_registSource" value="${param.search_EQ_registSource}">
			       	<input type="hidden"  id="v_search_EQ_level" value="${param.search_EQ_level}">
		      </form>
			</div>
	    ...
	    <div class="row"><!-- 操作按钮组 -->
		    <div class="col-md-12">
		      <div class="btn-group-sm pull-right mtb10">
		      	...
		      	<button value="1" type="button" class="searchState btn btn-default btn-sm"><span class="glyphicon glyphicon-search" aria-hidden="true"></span>&nbsp;正常</button>
		      	<button value="3" type="button" class="searchState btn btn-default btn-sm"><span class="glyphicon glyphicon-search" aria-hidden="true"></span>&nbsp;冻结</button>
		      	<shiro:hasPermission name="ssouser:create">
			        <a class="btn btn-primary" href="${ctx}/ssouser/sign" ><span class="glyphicon glyphicon-plus"></span> 用户添加</a>
		      	</shiro:hasPermission>
		      	<shiro:hasPermission name="ssouser:import">
		      	     <a class="btn btn-primary" href="#"  id="importSsouser"><span class="glyphicon glyphicon-plus"></span>批量导入用户</a>
		      	</shiro:hasPermission>
			...
			<table id="contentTable" class="table table-bordered table-condensed table-hover">
				...
				<c:forEach items="${data.content}" var="ssoUser" varStatus="stat">
					<tr>
						...
						<td>
							<a href="${ctx}/ssouser/view?action=view&id=${ssoUser.id }" >${ssoUser.qiuyouno }</a>
						</td>
						...
						<td><c:if test="${ssoUser.registSource == '0'}" >
							...
							<c:if test="${ssoUser.registSource == '1'}" >
								微信
							</c:if>
							<c:if test="${ssoUser.registSource == '2'}" >
								QQ
							</c:if>
							<c:if test="${ssoUser.registSource == '3'}" >
								新浪微博
							</c:if>
							<c:if test="${ssoUser.registSource == '4'}" >
								网站
							</c:if>
							<c:if test="${ssoUser.registSource == '5'}" >
								平台
							</c:if>
							<c:if test="${ssoUser.registSource == '6'}" >
								球馆
							</c:if>
						</td>
		         		<td>
		         			<fmt:formatDate value="${ssoUser.registTime }" pattern="yyyy-MM-dd"/>
		         		</td>
		         		<td><c:if test="${ssoUser.activeState == '0'}" >
								未激活
							</c:if>
							<c:if test="${ssoUser.activeState == '1'}" >
								已激活
							</c:if>
						</td>
						<td>
							<fmt:formatDate value="${ssoUser.createTime }" pattern="yyyy-MM-dd"/>
						</td>
						<td>
						  <a href="${ctx}/ssouser/view?action=view&id=${ssoUser.id }">查看</a>
						<!--   <shiro:hasPermission name="ssouser:delete">
						  	<a href="#" onclick="deleteById('${ssoUser.id }')"> 删除</a>
						  </shiro:hasPermission> -->
						  <c:if test="${ssoUser.state == '1'}" >
							  <shiro:hasPermission name="ssouser:freezeSsoUser">
							  	<a href="#" onclick="freezeSsoUser('${ssoUser.id }')"> 冻结</a> 
							  </shiro:hasPermission>
							  <shiro:hasPermission name="ssouser:sendMsg">
							  	<a href="#" onclick="sendSms('${ssoUser.phone}')"> 发送短信</a> 
							  </shiro:hasPermission>
						  </c:if>
						  <%-- <a href="#" onclick="sendCoupon('${ssoUser.id}')"> 领取新用户优惠券</a> --%> 
						  <c:if test="${ssoUser.state == '3'}" >
							  <shiro:hasPermission name="ssouser:unfreezeSsoUser">
							  	<a href="#" onclick="unfreezeSsoUser('${ssoUser.id }')"> 解冻</a> 
							  </shiro:hasPermission>
						  </c:if>
						  <c:if test="${lf:checkMarkeUser(ssoUser.id)}">
							  <a href="${ctx}/ssouser/addMarketUser/${ssoUser.id }">添加营销账号</a>
						  </c:if>
						</td>
					</tr>
				</c:forEach>
...
<div class="modal fade" id="couponModel" order_id="" tabindex="-1" role="dialog"
		aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
				...
				<div class="modal-footer">
					<a href="javascript:;" class="btn" data-dismiss="modal"
						aria-hidden="true">取消</a> <a href="javascript:;"
						class="btn btn-primary alert-to-ok" onClick="receiveCoupon()">领取</a>
			</div>
		</div>
	</div>
</div>
...
$(function() {
	  ...
	  $("#importSsouser").click(function(){
		$("#myDlgBody_lg").load("${ctx}/ssouser/importPhone_dlg");
		$("#myDlg_lg").modal();
	  });
...
function receiveCoupon(){
	...
	var id = $("#user_id").val();
	if(city==''){
		bootbox.alert('请输入所在城市！');
		return;
	}else{
		if(city.indexOf("市")==-1){
			bootbox.alert('城市名称要包含"市"');
			return;
		}
	}
	 bootbox.confirm('你确定要领取新用户优惠券吗？', function(result) {
	    if(result) {
	    	$.ajax({
		        cache: true,
		        type: "GET",
		        url:'${ctx}/ssouser/sendCoupon/'+id+'/'+city,
		        data:{},
		        async: false,
		        error: function(request) {
		        	common.showMessage("获取优惠券失败！");
		        },

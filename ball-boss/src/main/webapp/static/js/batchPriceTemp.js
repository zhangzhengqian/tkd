...
//绑定json
function BindAndEditWrap(data,id,action){
    var html = [];//保存html
    var timeNow = new Date().getTime().toString();//当前时间戳
    if (data && data.length && (Object.prototype.toString.call(data).indexOf('Array') > 0)) {
        $.each(data, function (i, v) {
            html.push('<div class="wrap" space_id="'+id+'" ballTypeCode="' + v['ballType'] + '">');
    ...
    $('#contentByTemplate_'+id).html(html.join(''));
    if(action=='detail'){
    	$('#contentByTemplate_'+id).find("input[type=text]").attr("disabled",true);
    }
}
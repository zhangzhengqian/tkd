  ...
  <div class="panel-footer">
			<div class="row">
				<div class="col-sm-12 text-right">
						  	 <a class="btn btn-sm btn-default" href="${ctx }/orders/nuomiList"><span class="glyphicon glyphicon-remove"></span> 返回</a>
				</div>
			</div>	
	</div>
				...
				<tbody>
					<c:forEach items="${order.relatedOrders}" var="relatedOrder" varStatus="stat1">
						<c:forEach items="${relatedOrder.orderItemVoList}" var="orderItem" varStatus="stat2">
							<tr>
								<c:if test="${stat2.count==1}"><td <c:if test="${fn:length(relatedOrder.orderItemVoList)>1}">style="line-height:8.57142858"  rowspan="${fn:length(relatedOrder.orderItemVoList)}" </c:if> class="text-center"><a href="${ctx }/orders/nuomiView/${relatedOrder.id}">${relatedOrder.id }</a></td></c:if>

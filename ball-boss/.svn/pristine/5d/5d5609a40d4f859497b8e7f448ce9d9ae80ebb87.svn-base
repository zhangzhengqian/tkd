...
<div class="modal-header">
	...
	<h4 class="modal-title" id="role_form_dlg_title">活动信息</h4>
</div>
		...
		<div class="span6" id="selContain">
			<ul class="thumbnails">
				...
				<c:if test="${stat.last }" >
					...
					<input type="hidden" id="areaCode" name="areaCode" value="${activity.areaCode }">
				</c:if>
					<li class="span4">
						<div class="thumbnail" style="width:500px;">
							<input id="sourceId" type="hidden" name="sourceId" value="${activity.id }" />
							<img alt="300x200" src="${activity.photo }" />
							<div style=" width: 100%;
							  height: 20px;
							...
							<div>
								<p>
									<button type="button" class="btn btn-default source_selected" data-dismiss="modal" data-title="${activity.name }" data-id="${activity.id }">选择</button>
								</p>
...
function loadPush(){
	...
	var listCount = 0;
	$.get('${ctx}/push/pushActivity?count='+count + '&areaCode=' + areaCode,function(data){
		if(data!=null){
			var htmlData = "";
			var pushData = data["pushs"].content;
			listCount = pushData.length;
			if (listCount == 0){
				bootbox.alert('没有更多活动');
			}
			for(var i = 0; i < pushData.length; i ++){
				...
				"<div class=\"thumbnail\" style=\"width:500px;\">" +
					...
					"<img alt=\"300x200\" src=\"" + pushData[i].photo + "\" />" + 

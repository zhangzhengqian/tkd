  ...
  <div class="panel-body"><!-- 右侧主体内容 -->
	...
	<form id="inputForm" action="${ctx}/corps/save" method="post" class="form-horizontal" enctype="multipart/form-data">
		<fieldset>
			...
			<input type="hidden" class="form-control" id="captain" name="captain" value="${result.captainId}"/>
			<input type="hidden" class="form-control" id="id" name="id" value="${result.info.id}"/>
 		    <div class="form-group form-group-sm">
		       <label for="name" class="col-md-3 control-label"><span class="text-red">* </span>战队名称:</label>
		       <div class="col-md-6 has-feedback">
		         <input type="text" class="form-control" id="name" name="name" value="${result.info.name}" placeholder="请输入战队名称" />
		       </div>
		    </div>
		    <div class="form-group form-group-sm">
		       <label for="name" class="col-md-3 control-label"><span class="text-red">* </span>战队队长:</label>
		       <div class="col-md-6 has-feedback">
		       	 <span id="captainName">${result.info.captain}</span>
		       </div>
		    </div>
   		  <div class="form-group form-group-sm">
		     ...
		     <div class="col-md-6 has-feedback form-inline">
		     	<tags:zone id="area" name="area" value="" />${result.info.area}
		     </div>
		 </div>
		 <div class="form-group form-group-sm">
	         <label for="address" class="col-md-3 control-label"><span class="text-red">* </span>详细地址:</label>
	         <div class="col-md-6 has-feedback ">
	           <input type="text" class="form-control" id="activityArea" name="activityArea" value="${result.info.activityArea}" placeholder="请输入详细地址"/>
	         </div>
	     </div>
	     <div class="form-group form-group-sm">
	         ...
	         <div class="col-md-3 has-feedback ">
	           <select class="form-control" id="sportType" name="sportType">		
					...
					<c:forEach items="${lf:dicItems('SPORT_TYPE') }" var="item">
						<option <c:if test="${result.info.sportType eq item.itemCode }">selected</c:if>  value="${item.itemCode }" >--${item.itemName }--</option>
					</c:forEach>
			   </select>
	         </div>
	     </div>
	     <div class="form-group form-group-sm">
	         ...
	         <div class="col-md-6 has-feedback">
				...
				<!-- 保存图片 -->
				<input id="logo" name="logo" type="hidden" value="${result.info.logo}" />
				<!-- 显示图片 -->
	      ...
	      <div class="form-group form-group-sm">
	         ...
	         <div class="col-md-6 has-feedback ">
	           <input type="text" class="form-control" id="phone" name="phone" value="${result.info.phone}" />
	         </div>
	     </div>
	     ...
	     <div class="form-group form-group-sm">
	         ...
	         <div class="col-md-6 has-feedback ">
	           <textarea  rows="5" style="height:8em;" id="introduction" class="form-control" placeholder="请输入战队介绍" name="introduction">${result.info.introduction}</textarea>
	         </div>
	     </div>
	     <div class="form-group form-group-sm">
	    ...
	    <div class="form-group form-group-sm">
	    	...
	    	<table id="member_table" class="table table-bordered">
	    		...
	    		<c:forEach items="${result.mem}" var="member">
	    			<c:if test="${member.userId==null||member.userId!=result.captainId}">
	 					<tr>
							...
							<td>${member.age}</td>
							<td>${member.phone}</td>
							<td><a href="javascript:;" onclick="removeMember(this);">删除</a></td>
						</tr>
					</c:if>
				</c:forEach>
				</tbody>
...
var members_ = eval('('+'${result.mem_json}'+')');
var members = [];
$(function() {
	menu.active('#corps-man');
	upload({'id':'logoFile','icon_img':'logo_img','icon':'logo'});  // 球馆LOGO
	$.each(members_,function(key,val){
		if(val["userId"]==undefined||val["userId"]!='${result.captainId}'){
			members.push({logo:val["logo"],name:val["name"],sex:val["sex"]==0?"男":"女",age:val["age"],phone:val["phone"]});
		}
	});
	$('#inputForm').validate({

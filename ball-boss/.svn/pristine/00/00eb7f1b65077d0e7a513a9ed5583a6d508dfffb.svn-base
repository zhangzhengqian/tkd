			...
			<div class="row">
				...
				<div class="col-md-12">
					<form id="cta_form" class="form-horizontal"
						...
						<input type="hidden" name="search_EQ_state" id="search_EQ_state"
							value="${param.search_EQ_state}">
					</form>
				</div>
			</div>
			<!-- /查询条件 -->
					...
					<div class="btn-group-sm pull-right mtb10">
						<shiro:hasPermission name="cta:create">
							<a class="btn btn-primary btn-sm"
								href="${ctx}/ssouser/cta/create?action=create"><span
							...
							<tbody>
								<c:forEach items="${data.content}" var="info" varStatus="stat">
									...
									<tr class="${status_class }">
										...
										<td>${info.secMobile}</td>
										<td>${info.status}</td>

										<td><a
											href="${ctx}/ssouser/cta/view/${info.userId}?action=view"
											class="btn btn-default btn-sm" id="editLink-${info.userId}">查看</a>
											<span class="cutline"></span> <a
											href="${ctx}/ssouser/cta/edit/${info.userId}?action=edit"
											class="btn btn-default btn-sm" id="editLink-${info.userId}">修改</a>
											<span class="cutline"></span> <a
											href="${ctx}/ssouser/cta/delete/${info.userId}?action==delete"
											class="btn btn-default btn-sm" id="removeLink-${info.userId}"
											onclick="deleteById('${info.userId}')">删除</a> <span
											class="cutline"></span></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>

...
public class CarouselService extends AbstractCacheService {
	...
	public Page<Carousel> findCarouselList(PageRequest pageRequest, Map<String, Object> searchParams) {
		...
		try {
			...
			String type = (String)searchParams.get("LIKE_type");
			if(StringUtils.isNotBlank(type)){
				paras.put("type", type);
			}
			String position = (String)searchParams.get("EQ_position");
			if(StringUtils.isNotBlank(position)){
				paras.put("position", Integer.valueOf(position));
			}else{
				paras.put("position", 0);
			}
			String areaCode = (String)searchParams.get("EQ_areaCode");
			if(StringUtils.isNotBlank(areaCode)){
				paras.put("areaCode", areaCode);
			}else{
				paras.put("areaCode", "110000");
				searchParams.put("EQ_areaCode", "110000");
			}
			String sqlCount = FreeMarkerUtils.format("/template/carousel/carousel_count.ftl", paras);
			...
			if(total==0){
				return new PageImpl<>(new ArrayList<Carousel>(), pageRequest, 0);
			}
			String sqlList = FreeMarkerUtils.format("/template/carousel/carousel.ftl", paras);
	        ...
	        List<Map<String,Object>> carouselMaps = jdbcTemplate.queryForList(sqlList);
	        String results = MyGson.getInstance().toJson(carouselMaps);
	        Type classType = new TypeToken<List<Carousel>>() {}.getType();
	        List<Carousel> carousels = MyGson.getInstance().fromJson(results, classType);
	        return new PageImpl<>(carousels, pageRequest, total);
		} catch (Exception e) {
			e.printStackTrace();

package com.lc.zy.ball.domain.oa.mapper;

import com.lc.zy.ball.domain.oa.po.User;
import com.lc.zy.ball.domain.oa.po.UserCriteria;
import java.util.List;
import org.apache.ibatis.annotations.Param;

/**
* UserMapper
* 
* @author sl
* @version v1.0
* @copy pet
* @date 2016-04-15 18:07:50
*/
public interface UserMapper {
    /**
     * 根据条件查询记录总数
     */
    int countByExample(UserCriteria userCriteria);

    /**
     * 根据条件删除记录
     */
    int deleteByExample(UserCriteria userCriteria);

    /**
     * 根据主键删除记录
     */
    int deleteByPrimaryKey(String userId);

    /**
     * 保存记录,不管记录里面的属性是否为空
     */
    int insert(User user);

    /**
     * 保存属性不为空的记录
     */
    int insertSelective(User user);

    /**
     * 根据条件查询记录集
     */
    List<User> selectByExample(UserCriteria userCriteria);

    /**
     * 根据主键查询记录
     */
    User selectByPrimaryKey(String userId);

    /**
     * 根据条件更新属性不为空的记录
     */
    int updateByExampleSelective(@Param("user") User user, @Param("userCriteria") UserCriteria userCriteria);

    /**
     * 根据条件更新记录
     */
    int updateByExample(@Param("user") User user, @Param("userCriteria") UserCriteria userCriteria);

    /**
     * 根据主键更新属性不为空的记录
     */
    int updateByPrimaryKeySelective(User user);

    /**
     * 根据主键更新记录
     */
    int updateByPrimaryKey(User user);
}
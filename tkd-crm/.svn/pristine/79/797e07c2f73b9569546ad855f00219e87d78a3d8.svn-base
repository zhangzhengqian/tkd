		...
		<c:forEach items="${data.content }" var="class" varStatus="stat">
			<tr>
				...
				<td>
					...
					<a class="btn btn-default btn-sm" href="javascirpt:void(0);" onclick="deleteClass('${class.id}')"><i class="glyphicon glyphicon-edit"></i> 删除</a>
...
<script type="text/javascript">
	...
	function deleteClass(v) {
		swal({
			...
			$('#loading').show();
			$.ajax({
				url : "${crx }/infoManage/deleteClass",
				method : "POST",
				data : {"classId" : v},
				dataType: 'json',

...
<body>
							...
							<c:forEach items="${data.content}" var="ssoUser" varStatus="stat">
								...
								已激活
									...
									<td><fmt:formatDate value="${ssoUser.createTime }"
											pattern="yyyy-MM-dd" /></td>
									<td><a
										href="${ctx}/statiums/view?action=view&id=${ssoUser.id }">查看</a>
	...
	<script type="text/javascript">
		...
		function freezeSsoUser(id) {
			var $form = $('#actionForm');
			$form.attr('action', '${ctx}/statiums/freezeSsoUser/' + id);
		...
		function unfreezeSsoUser(id) {
			var $form = $('#actionForm');
			$form.attr('action', '${ctx}/statiums/unfreezeSsoUser/' + id);
		...
		function unbundling(id){
			var $form=$("#actionForm");
			$form.attr('action','${ctx}/statiums/unbundling/'+id);
		...
		$("#export_btn").click(function() {
			$("#ssoUserForm").attr("action", "${ctx}/statiums/exportExcel");

...
func.delFunction =  function (id, name) {
    ...
    bootbox.confirm('您确定要删除 "'+ name + '" 功能吗？', function(result) {
        if(result) {
            var $form = $('#actionForm');
            $form.attr('action', ctx + '/admin/org/delete/' + id);
            $form[0].submit();
            common.loading();
        }
    });
...
$(function() {
	...
	/************************ 配置功能树 ************************/
	var settings = new TreeSetting(ctx + '/admin/org/tree');
	...
	function loadForm(treeNode, addFlag) {
		...
		var url = ctx + '/admin/org/form?plain';
		if (addFlag)
	...
	function onDrop(event, treeId, treeNodes, targetNode, moveType, isCopy) {
		...
		if (pId != node.pId || seqNum != node.seqNum) {
			$.post(ctx + "/admin/org/change", {id:node.id, pId: pId, seqNum: seqNum}, function(data) {
				common.log(data);
			});
		}
	}

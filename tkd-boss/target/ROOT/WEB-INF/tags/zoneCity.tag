...
<script src="${ctx }/static/js/zone.js"></script>
<% 
...
<select class="<%=clazz %> zone_coll_2${tagskey}" <c:if test="${disabled }">disabled</c:if> ></select>
<%-- <select class="<%=clazz %> zone_coll_3${tagskey}" <c:if test="${disabled }">disabled</c:if> ></select> --%>
...
$(function() {
	var zone${tagskey} = (function() {
		...
		var setColl1 = function(code){
			...
			$(".zone_coll_1${tagskey}").append('<option value="">请选择</option>');
			for ( i in nodes1 ){
				code = nodes1[i];
		...
		var setColl2 = function(code){
			$(".zone_coll_2${tagskey}").empty();
			$(".zone_coll_2${tagskey}").append('<option value="">请选择</option>');
			if("" != code){
				$(".zone_coll_3${tagskey}").empty();
		...
		var setColl3 = function(code){
			$(".zone_coll_3${tagskey}").empty();
			$(".zone_coll_3${tagskey}").append('<option value="">请选择</option>');

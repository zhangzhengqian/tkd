	    ...
	    <div class="form-group form-group-sm">
            ...
            <div class="col-md-6 has-feedback form-inline">
           		  <div class="input-group">
                  		<input readonly type="text" id="pDate" name="pDate" placeholder="请输入发布时间" value="${news.pDate }"
                  			class="form-control" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:00',alwaysUseStartDate:true})"/>
 							<label for="startTime" class="input-group-addon"><i class="fa fa-calendar"></i></label>
                    </div>
            </div>
        </div>
        <div class="form-group form-group-sm">
				<label for="type" class="col-md-3 control-label"><span class="text-red"></span>新闻类型:</label>
			    <div class="col-md-6 has-feedback form-inline">
			    	<input type="hidden" id="type_hidden" value="${news.type }"/>
					<select class="form-control" name="type" id="type" >
						<%-- <option id="option1" value="0"<c:if test="${news.type==0 }">selected </c:if>>--国际新闻--</option> --%>
						<option id="option2" value="1"<c:if test="${news.type==1 }">selected</c:if>>--新闻资讯--</option>
						<option id="option3" value="2"<c:if test="${news.type==2 }">selected</c:if>>--赛事活动新闻--</option>
					</select>
			  	</div>
			</div>
